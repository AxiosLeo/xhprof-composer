...
class Report
{
    ...
    public function __construct(array $run_data)
    {
        $run_data = ArrayTool::instance($run_data);
    ...
    {
        return $format ? Tool::time_format($this->total_wall_time) : $this->total_wall_time;
    }

    public function getTotalMemory($format = true)
    {
        return $format ? Tool::byte_format($this->total_memory) : $this->total_memory;
    }

    public function getTotalCpuTime($format = true)
    {
        return $format ? Tool::time_format($this->total_cpu_time) : $this->total_cpu_time;
    }
